...
class TestTwoImageEventRecurrentRestorationModel(BaseModel):
    ...
    def feed_data(self, data):
        ...
        self.seq_name = data["image_name"]  # List !!
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            m = self.opt["val"].get(
                "max_minibatch", n
            )  # m is the minibatch, equals to batch size or mini batch size
            i = 0
            while i < n:
                j = i + m
                if j >= n:
    ...
    def single_image_inference(self, img, voxel, save_path):
        self.feed_data(
            data={"lq": img.unsqueeze(dim=0), "voxel": voxel.unsqueeze(dim=0)}
        )
    ...
    def nondist_validation(
        ...
        if with_metrics:
            self.metric_results_deblur = {
                metric: 0 for metric in self.opt["val"]["metrics_deblur"].keys()
            }
            self.metric_results_interpo = {
                metric: 0 for metric in self.opt["val"]["metrics_interpo"].keys()
            }
            self.metric_results_total = {
                metric: 0 for metric in self.metric_results_deblur.keys()
            }
            ...
            for frame_idx in range(imgs_per_iter):
                ...
                if with_metrics:
                    ...
                    if use_image:
                        # interpo
                        if (
                            frame_idx >= self.m and frame_idx < self.m + self.n
                        ):  # interpo
                            for (
                                name,
                                opt_,
                            ) in (
                                opt_metric_interpo.items()
                            ):  # name: psnr, ...; opt_: type, ...
                                metric_type = opt_.pop(
                                    "type"
                                )  # calculate_psnr or calculate_ssim
    ...
    def _log_validation_metric_values(self, current_iter, dataset_name, tb_logger):
        ...
        for metric, value in self.metric_results_total.items():
            self.metric_results_total[metric] = (
                self.metric_results_deblur[metric] * 2 * self.m
                + self.metric_results_interpo[metric] * self.n
            )
